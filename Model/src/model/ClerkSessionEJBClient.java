...
public class ClerkSessionEJBClient {
    public static void main(String [] args) {
        try {
            ...
            ClerkSessionEJB clerkSessionEJB = (ClerkSessionEJB)context.lookup("TMS-Using JEE-Model-ClerkSessionEJB#training.iqgateway.services.ClerkSessionEJB");
            for (TmOffence tmoffence : (List<TmOffence>)clerkSessionEJB.getTmOffenceFindAll()) {
                printTmOffence(tmoffence);
            }
            for (TmVehicledetails tmvehicledetails : (List<TmVehicledetails>)clerkSessionEJB.getTmVehicledetailsFindAll()) {
                printTmVehicledetails(tmvehicledetails);
            }
            for (TmOffenceDetails tmoffencedetails : (List<TmOffenceDetails>)clerkSessionEJB.getTmOffenceDetailsFindAll()) {
                printTmOffenceDetails(tmoffencedetails);
            }
            for (TmOwnerdetails tmownerdetails : (List<TmOwnerdetails>)clerkSessionEJB.getTmOwnerdetailsFindAll()) {
